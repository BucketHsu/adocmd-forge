...
- 來源編輯器新增游標定位同步：滑鼠點選或鍵盤移動游標時，Preview 會捲動到目前來源行所屬的最近可渲染段落。
- 首次開啟與重新渲染 Preview 後改以目前游標行定位；既有來源可見範圍與 Preview 回傳同步仍共用 sequence 防迴圈機制。
- 新增 PreviewManager 事件路由單元測試與 Extension Host 游標移動回歸測試。
...
- 修正從編輯器右上角執行 PDF／HTML 匯出時，VS Code 傳入的來源文件 URI 被誤判成匯出目的地，導致顯示「匯出目的地不可覆蓋來源文件」的問題。
- 匯出命令現在會區分編輯器標題列的來源文件 URI 與自動化呼叫指定的輸出 URI，並保留另存檔案與覆寫確認流程。

## 1.2.6 - 2026-08-09

### Changed

- 編輯器與 Outline 右上角的操作統一使用 VS Code Codicon；命令 `title` 保留為滑鼠提示與命令面板文字，避免標題列顯示完整文字而占用過多寬度。
